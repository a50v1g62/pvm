...
4. **Copiar archivos a entorno PVM**
...
2. **Crear el Archivo de Hosts de PVM**
...
4. **Añadir `localhost` al Host de PVM**
...
1. **Navega al Directorio del Proyecto**
En la primera terminal (donde está el proyecto), navega al directorio donde se encuentran los ejecutables:

   ```bash
   cd $HOME/proyecto_pvm
   ```

2. **Ejecutar el Programa Maestro**
   Finalmente, ejecuta el programa maestro, pasando los parámetros `inicio` y `fin` como argumentos. En este caso, calcularemos la suma de los números del 1 al 100:
